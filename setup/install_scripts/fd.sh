...
FILENAME="fd-${TAG}-${ARCH}-${FD_OS}.tar.gz"
...
TOOL_PATH="$(tar -tzf "${TMP_FILE}" | grep '/fd$' | head -n 1)"

if [[ -z "${TOOL_PATH}" ]]; then
    echo "Could not find fd inside ${TMP_FILE}"
    exit 1
fi
...
echo "✅ fd is now available in ${BIN_DIR}/fd"
